...
var (
	providerName = "severity-filter"
	filterReason = "Filtered by severity filter"
)
		...
		if f.Finding.SeverityId >= s.minimumSeverity {
			enrichment := ocsffindinginfo.Enrichment{
				...
				Enrichment: &ocsffindinginfo.Enrichment_Filter{
					Filter: &ocsffindinginfo.Enrichment_FilterEnrichment{
						Reason: filterReason,
						Status: v1.IncidentFinding_VERDICT_ID_DISREGARD,
					},
				},
			}
			toBytes, err := protojson.Marshal(&enrichment)
			if err != nil {
				return nil, false, fmt.Errorf("failed to marshal enrichment err: %w", err)
			}
