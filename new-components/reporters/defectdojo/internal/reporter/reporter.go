...
func New(conf *Conf, client *client.Client) *dojoLogger {
	return &dojoLogger{
		conf:   conf,
		client: client,
	}
...
func (d dojoLogger) Report(ctx context.Context, findings []*vf.VulnerabilityFinding) error {
	...
	if err := d.handleResults(findings, findings[0].Finding.StartTimeDt); err != nil {
...
func (d dojoLogger) handleResults(findings []*vf.VulnerabilityFinding, scanStartTime *timestamppb.Timestamp) error {
	...
	engagement, err := d.client.CreateEngagement(scanUUID, startTime.Format(dojoTimeFormat), []string{"SmithyScan", d.conf.instanceID}, int32(d.conf.ProductID))
	if err != nil {
	...
	for toolName, findings := range vendoredFindings {
		for _, finding := range findings {
			...
			test, err := d.client.CreateTest(startTime.Format(dojoTestTimeFormat), toolName, "", []string{"SmithyScan", scanUUID}, engagement.ID)
			if err != nil {
			...
			for _, vuln := range finding.Finding.Vulnerabilities {
				...
				cweID := 0
				if vuln.Cwe != nil {
					c, err := strconv.ParseInt(vuln.Cwe.Uid, 10, 64)
					if err != nil {
				...
				_, err = d.client.CreateFinding(
					*vuln.Title,
					*description,
					severityToDojoSeverity(vuln.Severity),
					filePaths,
					startTime.Format(dojoTimeFormat),
					severityIDToDojoNumericalSeverity(vuln.Severity),
					[]string{"SmithyScan", scanUUID, toolName},
					test.ID, 0, int32(cweID), d.client.UserID,
					falsePositive,
					duplicate,
					cvssScore, // todo: why would a single vuln have multiple cvss?
				)
				if err != nil {
					return err
				}
...
func applyTemplate(finding *vf.VulnerabilityFinding, vulnerability *ocsf.Vulnerability) (*string, error) {
	...
	findingDescriptionTemplate := "{{.Vulnerability.VendorName}} found '{{.Vulnerability.Title}}'\n" +
