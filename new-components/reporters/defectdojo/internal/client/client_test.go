...
func TestDojoClient(t *testing.T) {
	...
	authToken := "foo"
	authUser := "bar"
	client, err := DojoClient(context.TODO(), mockTs.URL, authToken, authUser)
...
func TestCreateFinding(t *testing.T) {
	...
	input := types.FindingCreateRequest{
		Test:              1,
		FoundBy:           []int32{0},
		Duplicate:         false,
		Title:             "title",
	...
	_, err := c.CreateFinding(context.TODO(), input)
	require.NoError(t, err)
	assert.True(t, called)
}
...
func TestCreateEngagement(t *testing.T) {
	...
	c := &Client{host: mockTs.URL, apiToken: "test", user: ""}
	_, err := c.CreateEngagement(context.TODO(), "smithy scan foo", "2022-06-01", []string{"foo.git/somesha"}, 2)
	require.NoError(t, err)
	assert.True(t, called)
}
