...
func Test_ParseOut(t *testing.T) {
	t.Run("gosec testcases", func(t *testing.T) {
		...
		expectedIssues := []*v1.Issue{
			...
			},
			{
				...
				Severity:    v1.Severity_SEVERITY_HIGH,
				Confidence:  v1.Confidence_CONFIDENCE_UNSPECIFIED,
				Description: "Message: Use of weak random number generator (math/rand instead of crypto/rand) - nil endline",
			},
			{
				Target:      "file://main.go:83-83",
				Type:        "G404",
				Title:       "Use of weak random number generator (math/rand instead of crypto/rand) - nil startline",
				Severity:    v1.Severity_SEVERITY_HIGH,
				Confidence:  v1.Confidence_CONFIDENCE_UNSPECIFIED,
				Description: "Message: Use of weak random number generator (math/rand instead of crypto/rand) - nil startline",
			},
			{
				Target:      "file://main.go:347-347",
				Type:        "G104",
			...
			{
				Target:      "pkg:npm/%40angular/animation@12.3.1",
				Type:        "G104",
				Title:       "Vulnerable SQL injection code.",
				Severity:    v1.Severity_SEVERITY_HIGH,
				Confidence:  v1.Confidence_CONFIDENCE_UNSPECIFIED,
				Description: "Message: Vulnerable SQL injection code.",
			},
		}
		for _, run := range results.Runs {
			...
			issues, err := parseOut(*run, rules, "gosec", ExtraContextLanguageUnspecified)
			require.NoError(t, err)
			assert.EqualValues(t, expectedIssues, issues)
		}
	})
	t.Run("snyk-node testcases", func(t *testing.T) {
		...
		for _, run := range results.Runs {
			...
			issues, err := parseOut(*run, rules, "snyk-node", ExtraContextLanguageJS)
			require.NoError(t, err)
...
func Test_ToSmithy(t *testing.T) {
	...
	issues, err = ToSmithy(string(trivyOutput), ExtraContextLanguageUnspecified)
	require.NoError(t, err)

	expectedIssues := []*v1.Issue{
		{
			Target:      "file://library/ubuntu:1-1",
			Type:        "CVE-2016-20013",
