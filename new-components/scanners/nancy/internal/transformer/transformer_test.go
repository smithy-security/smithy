...
func TestNancyTransformer_Transform(t *testing.T) {
	var (
		clock = fakeClock()
	)
	...
	t.Run("it should transform correctly the finding to ocsf format", func(t *testing.T) {
		...
		ocsfTransformer, err := New(
			NancyRawOutFilePath("./testdata/nancy.json"),
			NancyTransformerWithTarget(ocsffindinginfo.DataSource_TARGET_TYPE_REPOSITORY),
			NancyTransformerWithClock(clock),
			NancyTransformerWithProjectRoot(filepath.Join(path, "../../")),
		)
		require.NoError(t, err)
		transformMethodTest(t, ocsfTransformer.Transform, nil, 4)
	})

	t.Run("it should error when it cannotfind the packages in go.mod files", func(t *testing.T) {
		path, err := os.Getwd()
		require.NoError(t, err)
		ocsfTransformer, err := New(
			NancyRawOutFilePath("./testdata/nancy-incorrect-packages.json"),
			NancyTransformerWithTarget(ocsffindinginfo.DataSource_TARGET_TYPE_REPOSITORY),
			NancyTransformerWithClock(clock),
			NancyTransformerWithProjectRoot(filepath.Join(path, "../../")),
		)
		require.NoError(t, err)
		transformMethodTest(t, ocsfTransformer.Transform, ErrCouldNotFindPackage, 0)
	})
}
