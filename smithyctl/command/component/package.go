...
func parseComponentSpec(path string) (*v1.Component, error) {
	const (
		defaultSmithyComponentFileNameYaml = "smithy-component.yaml"
		defaultSmithyComponentFileNameYml  = "smithy-component.yml"
	)

	// If the path is not defined we look in the current directory.
	if path == "" {
		path = "."
	}
	...
	var fullPath = path
	// If the path is a directory we check if it contains a smithy-component.yaml or smithy-component.yml.
	if info.IsDir() {
		...
		var numSmithyConfFiles uint
		for _, f := range files {
			if slices.Contains(
				[]string{
					defaultSmithyComponentFileNameYaml,
					defaultSmithyComponentFileNameYml,
				},
				f.Name(),
			) {
				numSmithyConfFiles++
		...
		if numSmithyConfFiles != 1 {
			return nil, fmt.Errorf("unexpected number of smithy-components found. Found %d but expected 1", numSmithyConfFiles)
		}
	}
	...
	if err := yaml.NewDecoder(bytes.NewReader(b)).Decode(&component); err != nil {
		return nil, fmt.Errorf("failed decode file '%s': %w", fullPath, err)
	}
