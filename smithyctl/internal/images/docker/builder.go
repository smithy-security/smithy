...
func (b *Builder) Build(ctx context.Context, cr *images.ComponentRepository) (string, error) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "build component image %s successfully\n", cr.URL())
	}
	if errLine.Error != "" {
		return "", errors.Errorf("%s: there was an error while building component image: %w", cr.URL(), errors.New(errLine.Error))
	}
