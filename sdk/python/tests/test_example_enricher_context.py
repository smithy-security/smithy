...
from smithy_python.components.example_enricher_context import main as example_enricher_context_main
...
class ExampleEnricherContextTest(unittest.TestCase):
    ...
    def test_example_enricher_context(self):
        ...
        self.assertEqual(len(findings), 5, "There should be five findings after our update retrieved from the remote database")
        for finding in findings:
            self.assertEqual(finding.details.comment, "This is a test comment.", "Each finding should have the expected annotation comment")
