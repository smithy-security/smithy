...
class TestRemoteDBManager(unittest.TestCase):
    ...
    def test_constructor_valid_parameters(self):
        ...
        db_manager = RemoteDBManager(
            instance_id=self.instance_id_with_data, logger=log, **self.default_config
        )
        ...
        self.assertEqual(db_manager.max_attempts, self.default_config["max_attempts"])
        self.assertEqual(db_manager.page_size, self.default_config["page_size"])
    ...
    def test_get_findings_empty_dataset(self):
        ...
        db_manager = RemoteDBManager(
            instance_id=self.instance_id_empty, logger=log, **self.default_config
        )
    ...
    def test_get_findings_with_pagination(self):
        ...
        db_manager_small = RemoteDBManager(
            instance_id=self.instance_id_with_data, logger=log, **small_page_config
        )
    ...
    def test_create_findings_success(self):
        ...
        db_manager = RemoteDBManager(
            instance_id=self.instance_id_empty,  # Use empty instance
            logger=log,
            **self.default_config,
        )
