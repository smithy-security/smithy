...
type (
	localStoreManager struct {
		clock clockwork.Clock
		dsn   string
		db    *sql.DB
	}

	localStoreManagerOption func(*localStoreManager) error

	// ErrInvalidConstructor should be used for invalid localStoreManager constructor errors.
...
// ManagerWithClock allows customising localStoreManager's clock.
func ManagerWithClock(clock clockwork.Clock) localStoreManagerOption {
	return func(m *localStoreManager) error {
		if clock == nil {
...
// NewLocalStoreManager returns a new SQLite database localStoreManager.
func NewLocalStoreManager(opts ...localStoreManagerOption) (*localStoreManager, error) {
	var (
		mgr = &localStoreManager{
...
func (m *localStoreManager) Validate(*ocsf.VulnerabilityFinding) error {
	return nil
}

// Read finds Vulnerability Findings by instanceID.
// It returns ErrNoFindingsFound is not vulnerabilities were found.
func (m *localStoreManager) Read(ctx context.Context, instanceID UUID) ([]*ocsf.VulnerabilityFinding, error) {
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, errors.Errorf("%s: %w", instanceID.String(), ErrNoFindingsFound)
		}
...
func (m *localStoreManager) Write(ctx context.Context, instanceID UUID, findings []*ocsf.VulnerabilityFinding) error {
...
func (m *localStoreManager) Update(ctx context.Context, instanceID UUID, findings []*ocsf.VulnerabilityFinding) error {
	jsonFindings, err := m.marshalFindings(findings)
	if err != nil {
		return err
	}
...
func (m *localStoreManager) Close(ctx context.Context) error {
...
func (m *localStoreManager) migrate() error {
...
func (m *localStoreManager) marshalFindings(findings []*ocsf.VulnerabilityFinding) (string, error) {
